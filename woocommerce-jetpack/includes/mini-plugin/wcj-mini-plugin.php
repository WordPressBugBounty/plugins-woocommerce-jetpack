...
add_filter(
	...
	function ( $module_categories ) {
		$module_categories = array_filter(
			$module_categories,
			function ( $item ) {
				return in_array( $item, array( 'emails_and_misc', 'pdf_invoicing', 'shipping_and_orders', 'payment_gateways', 'cart_and_checkout', 'products', 'labels', 'prices_and_currencies', 'labels', 'marketings', 'pdf_invoicing', 'dashboard' ), true );
			},
