...
if ( ! class_exists( 'WCJ_Shipping_Options' ) ) :
	...
	class WCJ_Shipping_Options extends WCJ_Module {
			...
			if ( $this->is_enabled() ) {
				...
				if ( 'yes' === wcj_get_option( 'wcj_shipping_hide_if_free_available_all', 'no' ) ) {
					add_filter(
						...
						wcj_get_woocommerce_package_rates_module_filter_priority( 'shipping_options_hide_free_shipping' ),
						2
					);
				}
		 ...
		 * @return array
		 */
		public function show_most_expensive_shipping( $rates, $package ) {
		 ...
		 */
		public function free_shipping_by_product( $is_available, $package ) {
		 ...
		 *
		 * @param string $rates defines the rates.
		 * @param array  $package defines the package.
		 *
		 * @version 2.9.1
		 */
		 ...
		 */
		public function add_hide_shipping_if_free_available_fields( $settings ) {
